...
- **双模设计**：
  - **真实 API 模式**：通过 HTTP 调用即梦/火山引擎视频生成服务，返回 `provider_task_id`，后台任务以指数退避算法异步轮询状态，成功后回传 MP4 视频链接。
  - **高保真 Mock 模式**：如果现场未配置 API Key 或算力排队，立即秒级生成带有镜头编号（S01/S02/S03）、时间戳与提示词水印的动态 9:16 5秒示例视频流，保证任何网络环境下演示 100% 成功！
